...
st.markdown("""
    ...
    /* === LIGHT CARTOON CARD LAYOUT WITH COUNTER-ROTATION === */
    .section-tag {
        font-family: 'Arial Black', Gadget, sans-serif;
        font-size: 0.9rem;
        font-weight: 900;
    ...
    /* Icon turns in opposite direction (negative degree) on hover */
    ...
    <!-- HTML5 Canvas Human Impact Engine Script -->
    <script>
    let score = 0;
    let bubbles = [];
    let canvas, ctx;
    ...
    function initGame() {
        ...
        // Track mouse movement explicitly across the game block zone for hover popping!
        canvas.addEventListener('mousemove', handleHumanImpact);
    }
    ...
    // HUMAN IMPACT RADAR ENGINE: Triggers pop instantly on cursor collision
    function handleHumanImpact(e) {
        ...
        bubbles.forEach((b, index) => {
            ...
            // Pop directly when user runs cursor through a bubble sphere
            if (dist < b.radius + 2) {
                bubbles.splice(index, 1);
                score++;
...
st.markdown(f"""
    <div class="game-zone">
        ...
        <!-- Stable Interactive Canvas Component Block -->
