...
st.set_page_config(
    page_title="Harkiran's Mega Hub! ✨",
    page_icon="🌈",
    layout="centered"
)
...
st.markdown("""
    ...
    /* === FLOATING BUBBLE GENERATOR (Pure CSS Canvas Animation) === */
    .bubble-bg {
        position: fixed;
    ...
    .profile-frame {
        width: 120px;
        height: 120px;
        border-radius: 30% 70% 70% 30% / 30% 30% 70% 70%; /* Blobby shape */
        background: linear-gradient(45deg, #FF007F, #7B2CBF, #00F5D4);
        display: inline-flex;
        align-items: center;
        justify-content: center;
        padding: 5px;
        animation: blobMorph 6s ease-in-out infinite alternate, spinSlow 20s linear infinite;
    ...
    /* Dynamic Theme Coloring per Card via CSS Variables */
    ...
    <!-- Interactive Bubble Machine & Copy Script -->
    <script>
    let popCount = 0;
    function popBubbleEffect() {
        popCount++;
        document.getElementById('score-value').innerText = popCount;
        
        // Dynamic Pop Color Flash
        const zone = document.getElementById('game-zone');
        const colors = ['#FF007F', '#00F5D4', '#FFFF00', '#A855F7', '#FF9F1C'];
        zone.style.backgroundColor = colors[popCount % colors.length] + '22';
        setTimeout(() => zone.style.backgroundColor = 'rgba(255, 255, 255, 0.85)', 150);
    }
    ...
    <!-- Visual Layer Injection for Canvas Bubbles -->
    <div class="bubble-bg">
        <div class="bubble" style="left: 10%; width: 40px; height: 40px; animation-delay: 0s; animation-duration: 7s;"></div>
        <div class="bubble" style="left: 30%; width: 20px; height: 20px; animation-delay: 2s; animation-duration: 9s;"></div>
        <div class="bubble" style="left: 55%; width: 50px; height: 50px; animation-delay: 1s; animation-duration: 6s;"></div>
        <div class="bubble" style="left: 80%; width: 30px; height: 30px; animation-delay: 4s; animation-duration: 8s;"></div>
    </div>
""", unsafe_allow_html=True)

# 3. Interactive Profile Banner Header
# Drop your image path/URL directly into the img src block below!
import base64

# Read your local image file and convert it to a browser-safe base64 string
try:
    with open("AVATAR.jpg", "rb") as image_file:
        encoded_string = base64.b64encode(image_file.read()).decode()
    MY_PHOTO_URL = f"data:image/jpeg;base64,{encoded_string}"
except FileNotFoundError:
    # Fallback image if the file name is misspelled or missing
    MY_PHOTO_URL = "https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=200&q=80"
st.markdown(f"""
    <div class="game-zone" id="game-zone" onclick="popBubbleEffect()">
...
# 4. Colorful Row Component Generator
...
# 5. Rendering Content Directories
